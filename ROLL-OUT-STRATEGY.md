## RKWALIFY Roll Out Stategy
 ...
 1. **This document only focuses on the initial 2026 release: 1.4.0.**

 2. Naming: Use "rkwalify" as repo and gem name, but use
    "kwalify" inside code and tests.  Use "Rkwalify" in documentaiton.

 3. Step 1: First add https://rubygems.org/gems/kwalify (0.7.2) into
    "rkwalify" repo. This includes all files (code, docs, etc).
    See commits for more details.
    * Assume this original "kwalify" gem will stay around.
    * Add credit to repo.

 4. Step 2: Then merge in https://rubygems.org/gems/dc-kwalify
    (1.0.0) to "rkwalify" repo. See commits for more details.
    * Add credit to repo.

 5. Step 3: Modernize the tool chain (bundler, rubygems, rvm, Rakefile,
    several rubocop gems, markdown, ruby 3.4.7, simplecov).
    See commits for more details. Also used my personal tools as needed.

 6. Create "rkwalify" issues for major work items.
    * More details at https://github.com/jasnow/rkwalify/issues .

 7. Initial Release Target
    -1- rkwalify's "test" Githut Action is green. Comment out test/test.rb
        tests that are broken. See [issue](https://github.com/jasnow/rkwalify/issues/2] for details.

    -2- ruby-advisory-db (Ruby 3.4.7) tests are green  using rkwalify code.
